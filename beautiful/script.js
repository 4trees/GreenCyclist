...
var svg = d3.select('#canvas').append('svg')
	.attr('width',w).attr('height',eachH * timePoint.length)
//update
...
var enterSeason = updateSeason.enter().append('g').attr('class',d => `season ${d.key}`)
	.attr('transform',(d,i) => `translate(${i * w / 4},0)`)
...
var updatetrees = enterSeason.append('g').attr('class','trees').selectAll('.tree').data(d => d.value.trees),
	entertrees = updatetrees.enter()
    	...
    	.attr('transform',d => `translate(${d.x * w / 4},${d.y * eachH * .7})`)
...
var timelabelContainer = enterSeason.append('g').attr('class','timelabel').attr('transform',`translate(${w / 8},${eachH * .9})`)
timelabelContainer.append('text').html(d => `<tspan x=0 dy=20>${d.value.season}</tspan><tspan x=0 dy=20>${d.value.time}</tspan>`)
